...
function modicAdminInfo(adminId,data){
    return new Promise((resolve,reject)=>{
        AdminDao.updateOne({adminId},{$set:data}).then(res=>{
            console.log(res);
            if(res.nModified){
        ...
        AdminDao.findOne({adminId}).then(res=>{
            if(res.password===oldPsd){
                if(res.password==='123456'){
                    return AdminDao.updateOne({adminId},{password:newPsd,status:'1'});
                }
                    return AdminDao.updateOne({adminId},{$set:{password:newPsd}});
            }else{
                resolve("旧密码错误");
            }
        }).then(res=>{
...
function findAllAdmin(pageNum){
    return new Promise((resolve,reject)=>{
        AdminDao.find({$or:[{power:'0'},{power:'1'}]}).skip((pageNum -1) * 8).limit(8).then(res=>{
            resolve(res)    
        }).catch(err=>{
            reject(err);
        })
...
function modicPower(adminId,power){
    return new Promise((resolve,reject)=>{
        AdminDao.updateOne({adminId},{power}).then(res=>{
            if(res.nModified){
                resolve(true);
            }else{
                resolve("未修改");
...
function modicStatus(adminId,status){
    return new Promise((resolve,reject)=>{
        AdminDao.updateOne({adminId},{status}).then(res=>{
            if(res.nModified){
                resolve(true);
            }else{
                resolve("未修改");
