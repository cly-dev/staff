...
export default function noticeList() {
  const columns = [
    {
      title: '标题', dataIndex: 'title', key: 'title', onCell: () => {
        return {
          style: {
            maxWidth: 150,
            overflow: 'hidden',
    ...
    {
      title: '内容', dataIndex: 'content', key: 'content', onCell: () => {
        return {
          style: {
            maxWidth: 150,
            overflow: 'hidden',
    ...
    <section>
      <Table
        pagination={
          {
            total,
            onChange: page => { handleChangePageNum(page) }
          }
        }
        columns={columns}
        dataSource={listData}
      ...
      <Modal width={800} title="查看详情" visible={isModalVisible} onCancel={() => setIsModalVisible(false)} onOk={() => setIsModalVisible(false)} cancelText="退出" okText="确定">
        <Descriptions  bordered>
          <Descriptions.Item label="标题" span={3}>{content.title}</Descriptions.Item>
          <Descriptions.Item label="发布时间" span={3}>{content.createTime}</Descriptions.Item>
          <Descriptions.Item label="已读人数" span={3}>{num}</Descriptions.Item>
          <Descriptions.Item label="内容详情" span={3}>{content.content}</Descriptions.Item>
        </Descriptions>
