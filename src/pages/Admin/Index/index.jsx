...
import { Layout, Menu, Dropdown, notification, Avatar } from "antd";
import {
  UserOutlined,
  LaptopOutlined,
    ...
    this.menu = (
      <Menu onClick={this.onClick}>
        <Menu.Item key="1" icon={<UserOutlined />}>
          <Link to="/admin-index/个人信息">个人信息</Link>
        </Menu.Item>
      ...
      <section className="Aindex-container">
        <Layout style={{ height: "100vh" }}>
          <Header className="header">
            <section className="logo">员工管理系统-管理员</section>
            <section className="info">
              <Dropdown overlay={this.menu}>
                <span className="info-name">
                  <Avatar src={this.info.imgPath} shape="square"></Avatar>
                  &nbsp;
                  {this.info.name}-{this.info.position}
          ...
          <Layout>
            <Sider width={200} className="site-layout-background">
              ...
              >
                ...
                >
                  <Menu.Item key="添加员工">
                    <Link to="/admin-index/员工管理/添加员工">添加员工</Link>
                  </Menu.Item>
                  <Menu.Item key="查看员工">
                    <Link to="/admin-index/员工管理/查看员工">查看员工</Link>
                  </Menu.Item>
                  <Menu.Item key="查看业绩">
                    <Link to="/admin-index/员工管理/查看业绩">查看业绩</Link>
                  </Menu.Item>
                  <Menu.Item key="搜索">
                    <Link to="/admin-index/员工管理/搜索">搜索</Link>
                  </Menu.Item>
                </SubMenu>
                <SubMenu
                  key="通知"
                ...
                >
                  <Menu.Item key="发布通知">
                    <Link to="/admin-index/通知/发布通知">发布通知</Link>
                  </Menu.Item>
                  <Menu.Item key="查看通知">
                    <Link to="/admin-index/通知/查看通知">查看通知</Link>
                  </Menu.Item>
                  <Menu.Item key="审核">
                    <Link to="/admin-index/通知/审核">审核</Link>
                  </Menu.Item>
                </SubMenu>
                <SubMenu key="工作台" icon={<LaptopOutlined />} title="工作台">
                  <Menu.Item key="修改信息">
                    <Link to="/admin-index/工作台/修改信息">修改信息</Link>
                  </Menu.Item>
                  <Menu.Item key="修改密码">
                    <Link to="/admin-index/工作台/修改密码">修改密码</Link>
                  </Menu.Item>
                  {power(this.info.power) ? (
                    <>
                      <Menu.Item key="添加管理">
                        <Link to="/admin-index/工作台/添加管理">添加管理</Link>
                      </Menu.Item>
                      <Menu.Item key="注销">
                        <Link to="/admin-index/工作台/管理">管理</Link>
                      </Menu.Item>
                    </>
                  ) : (
                    ""
            ...
            <Layout style={{ padding: "0 20px 20px" }}>
              ...
              >
                <Suspense fallback={<h1>加载失败</h1>}>
                  <Route
                    path="/admin-index/员工管理/添加员工"
                    component={AddStaff}
                  ></Route>
                  <Route
                    path="/admin-index/员工管理/查看员工"
                    component={StaffList}
                  ></Route>
                  <Route
                    path="/admin-index/员工管理/查看业绩"
                    component={Performance}
                  ></Route>
                  <Route
                    path="/admin-index/员工管理/搜索"
                    component={Search}
                  ></Route>
                  <Route
                    path="/admin-index/通知/发布通知"
                    component={AddNotice}
                  ></Route>
                  <Route
                    path="/admin-index/通知/查看通知"
                    component={NoticeList}
                  ></Route>
                  <Route
                    path="/admin-index/通知/审核"
                    component={ApplyList}
                  ></Route>
                  <Route
                    path="/admin-index/个人信息"
                    component={AdminInfo}
                  ></Route>
                  <Route
                    path="/admin-index/工作台/修改信息"
                    component={changeInfo}
                  ></Route>
                  <Route
                    path="/admin-index/工作台/修改密码"
                    component={changePassword}
                  ></Route>
                  <Route
                    path="/admin-index/工作台/添加管理"
                    component={AddAdmin}
                  ></Route>
                  <Route
                    path="/admin-index/工作台/管理"
                    component={AdminTer}
                  ></Route>
                </Suspense>
              </Content>
